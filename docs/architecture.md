...
- Plant profiles present a photo/identity hero, primary Water now action, secondary Snooze/Reminder actions, care basics rows, watering history, photo/identification, app reminder with compact Google Calendar status, edit, and archive surfaces.
...
Care profile helpers live in `src/lib/care-profiles`. They normalize names, derive safe scientific-name candidates for hybrid markers and binomial forms, derive genus fallback keys, load profile records with aliases, and match accepted identity in a conservative order: exact scientific name, scientific alias, synonym, common-name alias, genus profile, and explicit care-group alias. Ambiguous aliases return an `ambiguous` result and no plant fields are changed. Scientific-name hardening does not add broad fuzzy lookup, similarity scoring, or fuzzy common-name matching.

Add/Edit Plant can request an optional form-time care profile preview from a server action once reviewed common or scientific names are known. The client sends only the editable names or a selected basic-profile answer to the server action; browser code does not read care profile tables directly. A matched preview can appear in the Watering basics step with profile name, check cadence, optional range, dryness preference, watering guidance, and a home-conditions caveat. Users can explicitly apply the suggestion into the controlled `wateringIntervalDays` and `wateringGuidance` form fields or skip it before saving; no plant record, reminder, or calendar event is created by this preview.

When no form-time name match exists, Add/Edit Plant can offer a basic profile selection that maps to reviewed care-group or fallback profiles on the server. The basic profile flow does not identify the plant; it only returns the same optional watering starting point UI inside the form.

After a user explicitly saves a reviewed identification suggestion from the plant profile, the server attempts a care profile lookup using the saved scientific and common names. That profile identification panel can show matched, ambiguous, or no-match care states as immediate feedback from the explicit identification action. The normal plant profile does not show a persistent duplicate care setup panel after Add Plant save; care setup belongs in Add/Edit Plant watering basics. Matched care suggestions show the profile name, check cadence, optional range, dryness preference, watering guidance, and a home-conditions caveat. Users can explicitly use the suggested care basics, edit first, or skip for now. If the app-owned care profile tables are empty, unavailable, or stale compared with the validated in-app fixture coverage, the server logs a care-profile diagnostic and falls back to the validated fixtures instead of silently disabling suggestions.
...
Wave 1, Wave 2, Wave 3, the follow-up library expansion, the gap-first alias/profile patch, and the QA scientific-name patch seed coverage is represented as typed fixtures in `src/lib/care-profiles/fixtures.ts`. `npm run validate:care-profiles` validates required fields, cadence ranges, duplicate aliases, and intentionally ambiguous aliases. `npm run generate:care-profile-seed` regenerates `supabase/seed_care_profiles.sql` from those fixtures. Current coverage includes 123 profiles and 411 aliases, including common retail aliases, additional species profiles, genus-level fallbacks, orchid starters, and intentional ambiguous aliases for unsafe common-name collisions.
...
When a new plant is saved with reviewed common or scientific names, Add Plant redirects to the owned plant profile with an optional care review state only when still useful. The Add Plant Watering basics step can already show and apply a matched internal care profile before save, so users do not have to wait for the profile page to see a recommended starting cadence. This care step is optional: plant creation does not require AI, care suggestions, reminders, or successful photo upload.
