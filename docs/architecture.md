...
Add Plant can request an optional pre-save care profile preview from a server action once reviewed common or scientific names are known. The client sends only the editable names to the server action; browser code does not read care profile tables directly. A matched pre-save preview can appear in the Watering basics step with profile name, check cadence, optional range, dryness preference, watering guidance, and a home-conditions caveat. Users can explicitly apply the suggestion into the unsaved `wateringIntervalDays` and `wateringGuidance` form fields or skip it; no plant record, reminder, or calendar event is created by this preview.
